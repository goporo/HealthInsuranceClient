      ...
      <div className="h-28 bg-white flex flex-row p-4 gap-2 min-w-[250px]">
        ...
        <div className="">
          ...
          <p className="line-clamp-2">
            ...
            obcaecati autem sed assumenda. Ad consequatur eum saepe quas,
            dolorum rerum voluptatem possimus quasi aecati autem shred res?
          </p>
        </div>
      </div>
    </Link>
...
const ProductItem = ({ response, fetchLoading = false }) => {
  return (
    <>
      {!fetchLoading ? (
        response.map((product, idx) => renderProductItem(product, idx))
      ) : (
        <UiSpinning />
      )}
    </>
